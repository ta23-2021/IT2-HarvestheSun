jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					superpressAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			superpressGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				superpressGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			superpressGetAllInputs($(this).parent().parent().parent());
		}
	});
	// Add new item
	$('.customize-control-sortable-repeater-add').click(function(event) {
		event.preventDefault();
		superpressAppendRow($(this).parent());
		superpressGetAllInputs($(this).parent());
	});
	...
	function superpressAppendRow($element, defaultValue = '') {
	...
	function superpressGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  superpressGetAllImageCheckboxes($(this).parent().parent());
	});
	// Get the values from the checkboxes and add to our hidden field
	function superpressGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			superpressGetAllPillCheckboxes($(this).parent());
		}
	});
	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		superpressGetAllPillCheckboxes($(this).parent().parent().parent());
	});
	// Get the values from the checkboxes and add to our hidden field
	function superpressGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.superpressfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.superpressfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.superpressfontslist[index].category);
		superpressGetAllSelects($(this).parent().parent());
	});
	$('.google_fonts_select_control select').on('change', function() {
		superpressGetAllSelects($(this).parent().parent());
	});
	function superpressGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].superpresstinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].superpresstinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].superpressmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['superpress-upsell'] = api.Section.extend( {
