(function( $ ){
    ...
    var Raw_Framework_Sidebar = function() {
        ...
        this.template = $('#superpress-tmpl-widget');
        this.render_form();
        this.append_remove_button();
        this.bind_events();
    }
     Raw_Framework_Sidebar.prototype = {
        // gsuperpress our form and display it in the widgets screen
        ...
        append_remove_button: function() {
            ...
            var string = '<span class="superpress-remove-sidebar"><i class="dashicons dashicons-trash"></i></span>';
            area.find('.sidebar-superpress').append(string);
        },
         // bind events to the remove icon so that custom sidebars can be removed
        bind_events: function() {
            this.wrapper.on('click', '.superpress-remove-sidebar', $.proxy( this.delete_sidebar, this));
        },
         //delete the sidebar area with all widgets
        delete_sidebar: function(e) {
            // ask for the confirmation
            var process = confirm(superpressWidgets.sidebarConfirm);
            if(process == false) {
                return false;
            }
